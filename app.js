...
async function invokeDialogFlowAgent(msg,projectId = 'aari-test-agent-fafl') {
...
io.on('connection', socket => {
  socket.on('joinRoom', ({ username, room }) => {
   user = newUser(socket.id, username, room);
...
app.post('/send-cust-to-chatbot',async (req,res)=>{
 // console.log(req);
    console.log(req.body['customer-id']);
    res.send({message:req.body['customer-id']});
    var msg = "ID:"+req.body['customer-id'].split("Customer ID :")[1];
    io.to("Room").emit('message', formatMessage(user.username, msg));
    await  invokeDialogFlowAgent(msg).then(data=>{
      console.log(data);
